...
# Implement only: `src/gdc_adk/providers/google_provider.py`

## Owning subsystem
- `providers`

## Responsibility
Map the uniform provider contract into Google transport for cloud fallback only.

## Required public methods
- `generate(request)`
- `is_available()` only if supported by adapter design

## Requirements
- cloud use only after control-plane permission
- config-driven model resolution
- explicit secret handling
- normalized `LLMResponse`
- explicit provider failure surfacing
...
## Definition of done
- cloud provider remains thin, normalized, and reachable only through policy-approved fallback
