...
# Implement only: `src/gdc_adk/providers/ollama_provider.py`

## Owning subsystem
- `providers`

## Responsibility
Map the uniform provider contract into Ollama transport for local execution.
...
## Requirements
- config-driven base URL and model resolution
- normalized `LLMResponse`
- explicit failure for Ollama down
- explicit failure for bad base URL
- explicit failure for missing model
- explicit failure for non-200 response
- must not route around control plane
...
## Definition of done
- local provider transport is normalized, availability-aware, and subordinate to control-plane routing
