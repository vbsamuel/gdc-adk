...
| FX-R015 | Observability and replay | Every important run must be reconstructible from events/artifacts/issues/snapshots. | substrate/runtime/validation | event_spine, artifact_store, issue_tracker, memory | all above | operations, CI, review | replay contract | all | replay scenario tests | complete structured logs | replayability_gap | accepted |
| FX-R016 | External adapter typed boundary | Adapter and external interface surfaces must expose typed, serializable request/response boundaries, remain thin, preserve replay/reference envelopes, reject malformed external input before internal workflow logic, and prove Stage 6 -> Stage 7 contract-safe entry and replay-safe round trip. | adapters | `src/gdc_adk/adapters/adk/external_contracts.py`, `src/gdc_adk/adapters/adk/request_mapper.py`, `src/gdc_adk/adapters/adk/response_mapper.py`, `src/gdc_adk/adapters/adk/replay_envelope.py`, `src/gdc_adk/adapters/adk/weather_time_agent_adapter.py`, `labs/adk/weather_time_agent.py` | Stage 4 workflow contracts, Stage 5 continuity/replay contracts, Stage 6 handoff/coordination contracts | external ADK surfaces, future CLI/API adapters, replay/audit tooling | `ExternalRequest`, `ExternalResponse`, `AdapterInvocationContext`, `ExternalErrorResponse`, `ReplayReferenceEnvelope`, `ExternalValidationResult`, Stage 6 `Stage7ForwardEnvelope` | all externally-invoked modes | `tests/test_stage7.py`: external request/response contract tests; malformed input negative tests; replay-envelope round-trip tests; Stage 6 -> Stage 7 boundary proof; adapter misuse protection tests | adapter invocation trace; replay-envelope propagation trace; external validation failure records | adapter_business_logic_violation, adapter_boundary_contract_gap, replay_envelope_gap, stage6_boundary_bypass, memory_boundary_bypass, provider_boundary_leakage | accepted |
| FX-R015 | Observability and replay | Every important run must be reconstructible from typed structured records, preserving linkage across events, artifacts, issues, findings, snapshots, and replay envelopes where applicable. | substrate/runtime/validation | event_spine, dispatch_system, artifact_store, issue_tracker, memory, validation | all above | operations, CI, review | typed replay and observability contracts | all | bounded replay reconstruction, malformed/duplicate/inconsistent input rejection, cross-stage replay compatibility, and export/reset/load/replay lifecycle tests | typed structured replay evidence and reconstructible audit-safe replay bundles | replayability_gap | accepted |
...
### FX-R015
- status: accepted
- files: `src/gdc_adk/substrate/event_spine.py`, `src/gdc_adk/substrate/dispatch_system.py`, `src/gdc_adk/substrate/artifact_store.py`, `src/gdc_adk/substrate/issue_tracker.py`, `src/gdc_adk/memory/continuity.py`, `tests/test_stage8.py`
- contracts: `Event`, `Artifact`, `Issue`, `WorkflowRun`, `ContinuitySnapshot`, `ReplayEvidenceBundle`, `ReplayReconstructionResult`, Stage 4 `ReviewFinding`, and Stage 7 `ReplayReferenceEnvelope`
- workflow_modes: `all`, with bounded golden-path proof in `fix_flow`
- acceptance_scenarios: Scenario A, Scenario B, Scenario C, Scenario D, Scenario E
- evidence: `tests/test_stage8.py` proves bounded workflow reconstruction from typed events, artifacts, issues, findings, snapshots, and replay reference envelopes; explicit linkage preservation across workflow run, artifact, issue, finding, snapshot, and envelope references; export -> reset -> load -> replay round trip through public lifecycle APIs; malformed replay rejection; duplicate and inconsistent replay input rejection; missing required snapshot rejection; and partial replay payload rejection when full reconstruction is claimed. Integrated replay compatibility is exercised against `tests/test_stage1.py`, `tests/test_stage5.py`, `tests/test_stage6.py`, and `tests/test_stage7.py`.
- notes: `event_spine.py` now exposes public export/reset/load lifecycle APIs, and `dispatch_system.py` exposes typed, audit-safe replay reconstruction surfaces without absorbing workflow, memory, adapter, or governance business logic.
- disposition: accepted

### FX-R016
- status: accepted
- files: `src/gdc_adk/adapters/adk/external_contracts.py`, `src/gdc_adk/adapters/adk/request_mapper.py`, `src/gdc_adk/adapters/adk/response_mapper.py`, `src/gdc_adk/adapters/adk/replay_envelope.py`, `src/gdc_adk/adapters/adk/weather_time_agent_adapter.py`, `labs/adk/weather_time_agent.py`, `tests/test_stage7.py`
- contracts: `ExternalRequest`, `ExternalResponse`, `AdapterInvocationContext`, `ExternalErrorResponse`, `ReplayReferenceEnvelope`, `ExternalValidationResult`, and Stage 6 `Stage7ForwardEnvelope`
- workflow_modes: `all externally-invoked modes`
- acceptance_scenarios: Scenario A, Scenario B, Scenario C, Scenario D, Scenario E, Scenario F
- evidence: `tests/test_stage7.py` proves valid `ExternalRequest` acceptance and mapping into typed internal adapter surfaces; valid internal typed result mapping to `ExternalResponse`; replay/reference envelope round-trip serialization; malformed external input rejection; invalid replay/reference envelope rejection; invalid response mapping rejection; adapter misuse rejection; provider-internal and memory-internal leakage rejection; Stage 6 -> Stage 7 boundary proof using typed Stage 6 artifacts only; replay-safe adapter round trip; and thin adapter plus lab harness behavior without hidden business logic.
- notes: `external_contracts.py`, `request_mapper.py`, `response_mapper.py`, `replay_envelope.py`, `weather_time_agent_adapter.py`, and `labs/adk/weather_time_agent.py` keep the Stage 7 boundary typed, replay-safe, and thin with no workflow, provider, memory, or governance business logic hidden in adapters.
- disposition: accepted
